...
public:
  ...
  bool addEdge(int u, int v, int w); // added by Mao
...
void Graph<V>::printAdjacencyMatrix() const
{
  ...
  for (unsigned i = 0; i < neighbors.size(); i++)
  {
    for (Edge* e: neighbors[i])
    {
      ...
      adjacencyMatrix[i][e->v] = e->w; // added by Mao
    }
  }
...
//}
bool Graph<V>::addEdge(int u, int v, int w)
{
  return createEdge(new Edge(u, v, w)); // added by Mao
}
